...
router.get("/jobs", jwtAuth, async(req, res) => {
    ...
    let queryParams = {
        where: findParams,
        order: sortParams,
        include: [{ model: Admin, attributes: ["name"], required: true }],
    };
...
router.delete("/jobs/:id", jwtAuth, async(req, res) => {
    ...
    try {
        const job = await JOB.findOne({ where: { rid } });
        if (!job) {
...
// Admin/applicant gets all his applications [pagination]
router.get("/applications", jwtAuth, (req, res) => {
    ...
    if (user.type == "Admin") {
        Applications.findAll({
                where: { rid: user.uid },
                include: [{ model: JOB, order: ["dateOfPosting", "ASC"] }, { model: Admin }],
            })
            .then((applications) => {
                res.json(applications);
            })
...
// update status of application: [Applicant: Can cancel, Admin: Can do everything] [todo: test: done]
                        ...
                        Applications.count({
                            ...
                            if (activeApplicationCount < job.maxPositions) {
                                ...
                                application
                                    ...
                                    .then(() => {
                                        Applications.update({ status: "cancelled" }, {
                                            where: {
                                                applicationId: {
...
router.get("/applicants", jwtAuth, (req, res) => {
    ...
    if (user.type === "Admin") {
        ...
        Applications.findAll({
                where: { rid: user.uid },
                order: sortParams,
                include: [{ model: JOB }, { model: JobApplicant }],
            })
            .then((applications) => {
                        ...
                        .then(() => {
                            ...
                            Rating.findAll({
                                ...
                                .then((result) => {
                                    ...
                                    const avg = result[0].dataValues.average;
                                    console.log("nooooooo" + data.jobId);
                                    console.log("nooooooo" + avg);
                                    JOB.update({
                                            rating: avg,
                                        }, {
                                            where: { jid: data.jobId },
...
router.get("/rating", jwtAuth, (req, res) => {
    const user = req.user;
    console.log("fjfjjfskfjskjfkjsfj" + req.query.id);
    Rating.findOne({
        where: {
            senderId: user.uid,
            receiverId: req.query.id,
            category: user.type === "Admin" ? "applicant" : "job",
        },
    }).then((rating) => {
        if (rating === null) {
            res.json({
